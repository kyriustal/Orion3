...
import { UserPlus, Shield, ShieldCheck, ShieldAlert, Eye, MessageCircle, Settings, BarChart2, Loader2, Lock, CheckCircle2, XCircle, Trash2 } from "lucide-react";
...
export default function Team() {
  ...
  const [editFormData, setEditFormData] = useState({ name: "", role: "AGENT", password: "" });
  ...
  const handleOpenEditModal = (member: any) => {
    ...
    setEditFormData({ name: member.name, role: member.role, password: "" });
  ...
  const handleEditMember = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success("Membro da equipa atualizado!");
  ...
  const handleDeleteMember = async () => {
    ...
    try {
      ...
      setTeam(prev => prev.filter(m => m.id !== deletingMember.id));
      toast.success("Membro da equipa removido!");
  ...
  return (
    <div className="space-y-8 max-w-5xl">
                ...
                return (
                  <div key={member.id} className={`flex items-center justify-between p-4 rounded-xl border bg-white hover:bg-zinc-50 transition-colors ${r.borderColor}`}>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      {/* Botões de Ação para Administradores / Proprietários */}
                      {currentUser && (currentUser.role === 'OWNER' || currentUser.role === 'ADMIN') && (
                        <div className="flex items-center gap-1 ml-2">
                          {/* Só permitir acções se o alvo não for OWNER (se o actual for ADMIN) */}
        ...
        <div className="fixed inset-0 bg-zinc-950/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <Card className="w-full max-w-md shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            <form onSubmit={handleEditMember}>
              <CardHeader>
                <CardTitle>Editar Membro da Equipa</CardTitle>
                <CardDescription>Atualize os detalhes de acesso e permissões.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="space-y-2">
                ...
                <div className="space-y-2">
                  ...
                  />
                  <p className="text-[10px] text-zinc-400 italic">O e-mail de acesso não pode ser alterado.</p>
                </div>
                <div className="space-y-2">
                  <Label htmlFor="editPwd">Nova Senha (opcional)</Label>
                  <div className="relative">
                    <Input
                      id="editPwd"
                      type="password"
                ...
                <div className="space-y-2">
                  <Label htmlFor="editRole">Cargo / Nível de Acesso</Label>
                  <select
                    id="editRole"
                    title="Selecione o cargo do membro"
                    className="flex h-10 w-full rounded-md border border-zinc-200 bg-white px-3 py-2 text-sm focus-visible:outline-none focus:ring-2 focus:ring-emerald-500"
                    value={editFormData.role}
                    disabled={editingMember.email === currentUser?.email}
                  ...
                  {editingMember.email === currentUser?.email && (
                    <p className="text-[10px] text-amber-600">Não pode alterar a sua própria função de acesso.</p>
                  )}
                  {/* Preview do cargo selecionado */}
                  {editFormData.role && ROLES[editFormData.role as RoleKey] && (() => {
                    const r = ROLES[editFormData.role as RoleKey];
                    return (
                      <div className={`rounded-xl border p-3 mt-1 ${r.bgColor} ${r.borderColor}`}>
                        <div className="flex flex-wrap gap-2">
                          {r.permissions.map((p, i) => (
              ...
              <CardFooter className="bg-zinc-50 border-t border-zinc-100 py-4 flex justify-end gap-2 rounded-b-xl">
                ...
                <Button type="submit" disabled={isUpdating} className="min-w-[130px] bg-emerald-600 hover:bg-emerald-700">
                  {isUpdating ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Salvar Alterações'}
                </Button>
              </CardFooter>
            </form>
          </Card>
      ...
      {isDeleteConfirmOpen && deletingMember && (
        <div className="fixed inset-0 bg-zinc-950/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <Card className="w-full max-w-sm shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            <CardHeader>
              <CardTitle className="text-red-600">Excluir Membro da Equipa</CardTitle>
              <CardDescription>Esta ação é irreversível.</CardDescription>
            </CardHeader>
            <CardContent>
              <p className="text-sm text-zinc-600">
                Tem a certeza de que deseja remover <strong>{deletingMember.name}</strong> ({deletingMember.email}) da equipa? 
                O utilizador perderá imediatamente o acesso à plataforma.
              </p>
            </CardContent>
            <CardFooter className="bg-zinc-50 border-t border-zinc-100 py-4 flex justify-end gap-2 rounded-b-xl">
              ...
              <Button onClick={handleDeleteMember} disabled={isDeleting} className="bg-red-600 hover:bg-red-700 text-white">
